        ...
        if(ingame) {
            ...
            for (ProxiedPlayer p : ProxyServer.getInstance().getPlayers()) {
                if (p.hasPermission("miraireport.notify")) {
                    Title titles = ProxyServer.getInstance().createTitle().subTitle((new ComponentBuilder(ChatColor.translateAlternateColorCodes('&', config.getString("subtitle").replace("%reporter%", reporter)))).create());
                    Title title = ProxyServer.getInstance().createTitle().title((new ComponentBuilder(ChatColor.translateAlternateColorCodes('&', config.getString("title")))).create());
                    p.sendTitle(title);
                    p.sendTitle(titles);
